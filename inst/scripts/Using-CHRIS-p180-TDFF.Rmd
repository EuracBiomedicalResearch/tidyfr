...
In this particular case, 3 versions of the module are available. We can next
...
metabolite concentrations and a quality information flag for each individual measurement.
...
measurements (hence there are in total 350 columns + 1 column with the
identifier of the sample). Description and information on the individual columns
can be loaded with the `labels` function:
...
Each row corresponds to a single analyte (metabolite) providing also annotations
as its name(s), chemical formula(s) etc. Columns *cv_qc_chris* represent the
coefficient of variation (CV) calculated on QC samples, the QC CHRIS Pool
samples. This values thus represents the technical variability for each
metabolite in the present dataset.
...
```{r}
cn <- metabo_ann[match(colnames(metabo_data), metabo_ann$label), "description"]
cn[1] <- "aid"
colnames(metabo_data) <- cn
